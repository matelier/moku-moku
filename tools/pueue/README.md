...
これらシステムは、複数人で計算機を共用する際には必要ですが、網羅計算の自動実行制御には適さない場合があります。
...
原理的には、用途別に「専用キュー」を用意して使い分けることで解決しますが、キューの作成には管理者権限が必要です。
...
ディレクトリ名を工夫するなどすれば順番を一致させることができますので、必要に応じて工夫してください。
...
フロントエンドでデーモンを動かして、ジョブ管理システムへの投入をPueueで管理すれば実現できます。
そのような利用が許されることを、事前に計算機管理に確認してください。
