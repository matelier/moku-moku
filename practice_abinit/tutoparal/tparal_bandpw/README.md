...
ABINITでは、詳細指定なしに並列計算すると、[k点並列＋スピン並列が用いられることを確かめました](../../tutorial/tbasepar/README.md)。
この並列はシンプルでわかりやすいのですが、効率良い大規模並列には不十分です。

ABINITには、他にも並列計算のための仕組みが備わっています。
効率良い並列計算実行を目指して、それらの使い方を学びます。
...
この設定でチュートリアルに沿って実行すると、並列に関して説明文と定性的に異なる振る舞いが見られました。
...
（一部比較のために、オリジナルのカットオフエネルギーで実行しました。）
...
ここまで同一繰り返し回数での実行時間（繰り返し五回に要する計算時間）に注目してきました。
...
各行最後の数値に注目します。
この数値は残差であり、小さいほどよく収束していることを意味します。
...
`bandpp = 1`, `npband = 6`, `npfft = 4`の条件で計算します。

```C
 ETOT 10  -3672.7531274410    -8.188E-05 3.918E-05 2.426E-03
```

```C
- Proc.   0 individual time (sec): cpu=        542.8  wall=        542.9
```

`npband = 6`, `npfft = 4`はそのままに、`bandpp = 2`に変更します。
`npband` ✖️ `bandpp`は12です。

```C
 ETOT 10  -3672.7531321457    -6.521E-05 5.811E-05 1.526E-03
```

```C
- Proc.   0 individual time (sec): cpu=        535.4  wall=        535.4
```

さらに`npband = 6`, `npfft = 4`はそのままに、`bandpp = 4`に変更します。
`npband` ✖️ `bandpp`は24です。

```C
 ETOT 10  -3672.7531413559     2.989E-05 2.679E-05 4.114E-04
```

```C
- Proc.   0 individual time (sec): cpu=        535.9  wall=        536.0
```

`bandpp`を大きくするに従って残差の減少、すなわち収束性の向上が確認できます。
計算時間は長くなることがあるので、やたらに大きな値は避けた方が良いでしょう。

次に、`bandpp = 1`, `npband = 12`, `npfft = 2`に変更して計算してみます。
`npband` ✖️ `bandpp`は12です。
...
`npband` ✖️ `bandpp`が同じであれば、収束性も同じになります。
一方この組み合わせで計算時間が長くなることは、先に見た通りです。

```C
- Proc.   0 individual time (sec): cpu=        742.1  wall=        742.1
```

さらに、`bandpp = 2`, `npband = 12`, `npfft = 2`に変更して計算してみます。
`npband` ✖️ `bandpp`は24です。
...
収束性は先の`npband = 6`, `npfft = 4`, `bandpp = 4`と同じです。

```C
- Proc.   0 individual time (sec): cpu=        754.4  wall=        754.5
```

計算時間はさらに長くなりました。

これらパラメータを上手に設定して、計算時間と収束性を調整します。
