...
これを、縦・横・高さ方向に繰り返し配置することにより、無限に大きな「結晶」を扱います。
...
繰り返しの単位となる格子は、基本格子が三次元ベクトル表示で与えられています。
```
...
入力ファイルではBravais格子を与えて、基本格子で計算する設定はbcc-Feを参照してください。
...
このシリコン単結晶の例題は、極めて収束が容易ですので、詳細を気にする必要はありませんが、電子状態の収束が困難な場合はここを見直す必要が生じます。
...
本サンプルではあらかじめk点ファイル`kpoint.data`が用意されていますが、その作り方から説明します。

kpoint.dataファイルを生成するためのスクリプト`band_kpoint.pl`が、そしてこのスクリプトの与える入力ファイルのテンプレートが`../../../tools/bandkpt_fcc_xglux.in`に用意されています。
...
入力ファイル`input_band_Si.data`では、k点情報をファイルから読み込むように指定しています。
