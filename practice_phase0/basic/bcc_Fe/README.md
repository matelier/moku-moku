...
`lattice_system = basecentered`が、基本格子で計算するための本質的な指定です。
...
それぞれ更新されて、収束した時点では、アップスピン（多数派）が5.13個に対してダウンスピン（少数派）が2.87個と求まりました。
...
`!`以降はコメントとして扱われますので、`zeta`は既定値のゼロに設定されます。

計算実行すると、アップとダウンの電子数は4個ずつで等しく、スピン分極は初期値から有意に変化することなく、電子状態が収束しました。
...
一方、SCF計算の収束判定は全エネルギーの変化量ですので、占有準位のみが関与します。
