...
シェルスクリプトは、シェルの対話環境から実行するのが一般的なので、対話利用の基礎も説明します。
...
| mv| ファイルを移動します。ファイル名変更はこのコマンドを利用します。(move) |
...
テンプレートファイルを用意して、それを書き換えた計算実行用のファイルを作成する。
次のコマンドは、テンプレートファイルnfinput.data中の文字列「$1」を全て変数aの値で置き換えたnfinp.dataファイルを生成する。

``` sh
sed 's/$1/'$a'/g' nfinput.data > nfinp.data
```

正規表現を活用すると、高度な処理を実現できる。
例えば連続する空白文字（空白とタブ）を単一の空白文字に置き換えるには、以下のように利用する。

``` sh
sed -E 's/[ \t]+/ /g' input_file
```

置き換えた結果は、標準出力に書き出される。
...
現在は独立したプログラムとして実装されていることが多いようです。
bcはPOSIXで標準化されているので多くのシステムで利用できるのですが、**busyboxでは利用できません。**
また、busybox-w32のdcコマンドは、GNU dcとは動作が異なります。
...
PHASE/0の出力ファイルoutput000には、繰り返し計算の過程での全エネルギーが「TH」という文字列を含む（正規表現がマッチする）行に出力されています。
そこで、「TH」を含む行のみを抽出すると、全エネルギーの変化、すなわち収束の様子を確認することができます。
...
指標となる文字列と結果数値が異なる行に出力されている場合に利用します（例えばPHASE／0では応力値の抽出）。
...
これに関連してここでは、FeとMnもしくはCoとの1:1合金を模擬する単純な計算をしてみます。
...
体心格子の角にFeが配置され、中心にはMn, Fe Coが順番に配置される（31, 38行目の元素記号\$2、38行目の原子番号\$3）。
各元素について、格子定数を変えた（20行目\$1）計算が実行される。


様々な条件の計算を実行すると、中には収束し難い条件があるかも知れません。
収束しない計算は適当なところで打ち切って残りの計算を実行したいので、4行目でSCF計算回数を制限しています（この例では対称性により原子に力が作用しないため、電子状態が収束したとみなされると計算終了）。
...
/
```
上記二つのファイルから、以下のシェルスクリプトがPHASE/0の入力ファイルnfinp.dataとfile_names.dataを生成する。
...
単純な処理なので、全ての変数をグローバルにし、関数への引数を利用せず、グローバル変数でデータを渡しています。
...
この例は立方晶なので応力の比対角成分はゼロ、対角成分は全て同じ値になる）が\$file_stress（stressFe.txtなど）にそれぞれ書き出されます。
...
固体の全エネルギー$E$の体積$V$依存性は、マーナハンの式に従うことが知られています。

$$
E(V) = \frac{BV}{B'(B'-1)}\left[B'\left(1-\frac{V_\text{opt}}{V}\right) + \left(\frac{V_\text{opt}}{V}\right)^{B'} - 1\right] + E_\text{opt}
$$

ここで$B$は体積弾性率（$B'$はその圧力微分）、$V_\text{opt}$と$E_\text{opt}$は、最安定な格子体積とその全エネルギーです。
gnuplotを使って、計算結果volumeCo.txtをマーナハンの式にフィッティングして、最安定な格子定数を求めましょう。
volumeCo.txtを見れば、最安定な格子定数とその全エネルギーのおおよその見当がつくので、それをフィッティングの初期値に与えてから（4, 5行目のVとE）、6行目でフィッティングを実行します。
...
最後に、フィッティング結果を計算データと共にプロットします。
精度良くフィッティングできており、格子体積は23.1866Å$^3$（格子定数：2.85Å）と求まりました。
